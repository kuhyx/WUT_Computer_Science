...
class MatrixGenerator:
    ...
    def generate_spd_matrix(n: int) -> np.ndarray:
        ...
        return np.dot(A, A.T) + n * MatrixGenerator.generate_identity_matrix(
            n
    ...
    def generate_matrix_and_vector(type, size=None):
        if type == "spd":
            ...
            raise ValueError(
                "Invalid type specified. Choose 'spd', 'nemeth12', or 'poli3'."
            )
    ...
    @staticmethod
    def save_to_file(matrix, vector, lambda_min, lambda_max, file_path) -> None:
        np.savez(
            file_path,
            matrix=matrix,
            vector=vector,
