...
def matrix_vector_multiply(A, input_x, start, end, Ax) -> None:
    Ax[start:end] = [sum(x * y for x, y in zip(row, input_x)) for row in A[start:end]]


@time_measurement_longest(longest_threads_time_accumulator)
def vector_vector_subtraction(b, Ax, start, end, residual) -> None:
    residual[start:end] = [x - y for x, y in zip(b[start:end], Ax[start:end])]


@time_measurement_longest(longest_threads_time_accumulator)
def scalar_vector_multiply(omega, vector, start, end, result) -> None:
    result[start:end] = [omega * x for x in vector[start:end]]


@time_measurement_longest(longest_threads_time_accumulator)
def vector_vector_addition(input_x, vector, start, end, output_x) -> None:
...
def RichardsonMethodThreads(
    ...
    with ThreadPoolExecutor(
        ...
        for iteration in range(max_iterations):
            Ax = [0] * len(
                x
            )  # tutaj zostanie przypisany wynik z mnożenia macierzy A z wektorem x
            futures = []
            ...
            for i in range(num_threads):
                ...
                futures.append(
                    executor.submit(matrix_vector_multiply, A, x, start, end, Ax)
                )
            ...
            residual = [0] * len(
                b
            )  # tutaj zostanie przypisany wynik z vector_vector_subtraction
            futures = []
            ...
            for i in range(num_threads):
                ...
                futures.append(
                    executor.submit(
                        vector_vector_subtraction, b, Ax, start, end, residual
                    )
                )
            ...
            change_vector = [0] * len(
                residual
            )  # zostanie tu przypisany wynik scalar_vector_multiply po pracy wątków
            futures = []
            ...
            _x = x[
                :
            ]  # do _x zostanie przez wątki przypisany wynik pracy w danej iteracji
            futures = []
            ...
            for i in range(num_threads):
                ...
                futures.append(
                    executor.submit(
                        vector_vector_addition, x, change_vector, start, end, _x
                    )
                )
