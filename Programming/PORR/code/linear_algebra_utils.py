...
class LinearAlgebraUtils:
    ...
    @staticmethod
    @abstractmethod
    def matrix_vector_multiply(A, x) -> None:
        pass

    @staticmethod
    @abstractmethod
    def vector_norm(v) -> None:
        pass

    @staticmethod
    @abstractmethod
    def vector_scalar_divide(x, scalar) -> None:
        pass

    @staticmethod
    @abstractmethod
    def matrix_scalar_multiply(A, w) -> None:
        pass

    @staticmethod
    @abstractmethod
    def vector_vector_subtraction(v1, v2) -> None:
        pass

    @staticmethod
    @abstractmethod
    def vector_vector_addition(v1, v2) -> None:
        pass

    @staticmethod
    @abstractmethod
    def scalar_vector_multiply(omega, vector) -> None:
        pass

    @staticmethod
    @abstractmethod
    def matrix_norm(A) -> None:
        pass

    @staticmethod
    @abstractmethod
    def matrix_matrix_subtraction(A, B) -> None:
        pass
...
class ProcessLinearAlgebraUtils:
    ...
    def dot_product(v1, v2):
        with Pool() as pool:
            result = pool.starmap(
                ProcessLinearAlgebraUtils.multiply_elements, zip(v1, v2)
            )
        return sum(result)
    ...
    def vector_vector_subtraction(v1, v2):
        with Pool() as pool:
            result = pool.map(
                ...
                zip(v1, v2, itertools.repeat(operator.sub)),
            )
        return list(result)
    ...
    def vector_vector_addition(v1, v2):
        with Pool() as pool:
            result = pool.map(
                ProcessLinearAlgebraUtils.vector_vector_operation,
                zip(v1, v2, itertools.repeat(operator.add)),
            )
        return list(result)
...
class DistributedArraysLinearAlgebraUtils(ABC):
    ...
    def gaussian_elimination(A, b):
        try:
            ...
            logger.info("Error during Gaussian elimination: %s", e)
            return None
